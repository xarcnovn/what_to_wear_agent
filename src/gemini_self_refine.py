...
class WeatherData:
    ...
    def __init__(self, location: str) -> None:
        ...
        self.nearest_cameras = [                    
            'https://imageserver.webcamera.pl/rec/krakow-florianska/latest.mp4']            # 'https://imageserver.webcamera.pl/rec/krakow-synagoga-stara/latest.mp4',
...
class SelfRefineAgent:
    ...
    def run(self):
        ...
        self.generate_initial_recommendation(['first image', self.images[0], initial_prompt]) # 'second image', self.images[1], 'third image', self.images[2]
        for _ in range(self.refinements):
            self.generate_feedback(feedback_prompt)
            self.refine_recommendation(refinement_prompt)
            time.sleep(10) #I keep getting 429 and decided to resolve it thixs way and it sucks guys
...
#     """
#     The app that takes output from WeatherData, recommends what to wear to be comfortable outside and sends an email to the user
#     """
...
#             self.weather_camera_images = weather_data_app.weather_camera_images
#             weather_data = [weather_data_app.weather_current, weather_data_app.weather_future]
#             recommendation_app = SelfRefineAgent(weather_data, self.weather_camera_images)
#             recommendation_app.run()
#             sender_client = SenderClient()
#             self.latest_recommendation = recommendation_app.refined_recommendation
#             sender_client.send_email(self.latest_recommendation)
#             self.thinking_history = recommendation_app.chat.history
